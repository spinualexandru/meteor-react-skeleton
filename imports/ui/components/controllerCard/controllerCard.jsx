...
export default class WorkerCard extends Component {
  ...
  componentDidMount() {
    ...
    Meteor.call('bot.controller.getStatus', this.props.ip, (err,data) =>{
        this.setState({
          botStatus: data
        })
    });
  ...
  onDetailedClick(data, ev) {
    FlowRouter.go('/bot/controllers/'+data._str)
  }
  ...
  onToggleStatus() {
    Meteor.call('bot.controller.getStatus', this.props.ip, (err,data) =>{
      ...
      HTTP.call('POST', `http://${this.props.ip}:3000/api/setStatus`,{
        data:{
          status: newStatus
        }
      }, (errr, dataa)=>{
      ...
      return (
        <Card>
          <Card.Content>
            ...
            <Card.Description>
              Use some of the tools below to control the controller.
            </Card.Description>
          </Card.Content>
          <Card.Content extra>
